    ...
    >
      <div class="modal-dialog">
        <div class="modal-content">
          ...
          <div class="modal-body">
            <p>
              Here is your new client secret. This is the only time it will be
              shown so don't lose it! You may now use this secret to make API
              requests.
            </p>
...
export default {
  ...
  methods: {
    ...
    persistClient(method, uri, form, modal) {
      ...
      window.axios[method](uri, form)
        .then(({ data }) => {
          ...
          if (data.plainSecret) {
            this.showClientSecret(data.plainSecret)
          }
        })
        .catch((error) => {
    ...
    showClientSecret(clientSecret) {
      ...
      window.$('#modal-client-secret').modal('show')
    },
